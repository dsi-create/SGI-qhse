...
export function PortalHero({
  ...
  return (
    <section
      className={cn(
        "relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white px-6 py-7 shadow-sm md:px-8 md:py-8",
        className,
      )}
    >
      ...
      <div className="pointer-events-none absolute -right-20 -top-24 h-56 w-56 rounded-full bg-cyan-100/50 blur-3xl" />
      <div className="pointer-events-none absolute -bottom-24 right-24 h-40 w-40 rounded-full bg-teal-100/40 blur-3xl" />
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.35]"
        style={{
          backgroundImage:
            "radial-gradient(circle at 1px 1px, hsl(199 89% 48% / 0.12) 1px, transparent 0)",
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0">
              ...
              {subtitle && (
                <div className="mt-1.5 text-base font-medium text-slate-600 md:text-lg">
                  {subtitle}
                </div>
              )}
              {description && (
